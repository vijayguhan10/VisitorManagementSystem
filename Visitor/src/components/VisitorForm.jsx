...
function VisitorForm({ onSubmitSuccess }) {
  ...
  const validateStep1 = () => {
    ...
    formData.companions.forEach((companion, index) => {
      if (companion.name.trim() && !companion.phoneNumber.trim()) {
        newErrors[`companion${index}Phone`] = 'Phone number is required for companion'
      }
      if (!companion.name.trim() && companion.phoneNumber.trim()) {
        newErrors[`companion${index}Name`] = 'Name is required for companion'
      }
      if (companion.phoneNumber.trim() && !phoneRegex.test(companion.phoneNumber.replace(/\D/g, ''))) {
        newErrors[`companion${index}Phone`] = 'Please enter a valid 10-digit phone number'
      }
    })
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-4 md:p-6 lg:p-8">
      <h2 className="text-xl md:text-2xl font-semibold text-neutral-800 mb-6">Visitor Registration</h2>
      ...
      {!showConfirmation ? (
        <form onSubmit={(e) => e.preventDefault()} className="max-w-2xl mx-auto">
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium text-neutral-800">Companions</h3>
                  <button
                    type="button"
                ...
                {formData.companions.map((companion, index) => (
                  <div key={index} className="p-4 bg-neutral-50 rounded-lg space-y-4">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-medium text-neutral-600">Companion {index + 1}</h4>
                      <button
                        type="button"
          ...
          {step === 2 && (
            ...
            >
              <div>
                <h3 className="text-lg font-medium text-neutral-800 mb-2">Take a Photo</h3>
      ...
      >
        <div className="bg-primary-50 rounded-lg p-4 sm:p-6 border border-primary-200">
          <h3 className="text-lg sm:text-xl font-semibold text-primary-800 mb-4">Confirm Visitor Information</h3>
      
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-neutral-500">Visitor Name</p>
              <p className="font-medium text-neutral-800 break-words">{formData.visitorName}</p>
            </div>
      
            <div>
              <p className="text-sm text-neutral-500">Phone Number</p>
              <p className="font-medium text-neutral-800 break-words">{formData.phoneNumber}</p>
            </div>
      
            <div>
              <p className="text-sm text-neutral-500">Reason for Visit</p>
              <p className="font-medium text-neutral-800 break-words">{formData.reason}</p>
            </div>
      
            <div>
              <p className="text-sm text-neutral-500">Address</p>
              <p className="font-medium text-neutral-800 break-words">{formData.address}</p>
            </div>
            ...
            {formData.companions.length > 0 && (
              <div className="col-span-1 md:col-span-2">
                ...
                <div className="space-y-2 max-h-60 overflow-auto pr-1">
                  {formData.companions.map((companion, index) => (
                    <div key={index} className="bg-white rounded p-3 border border-neutral-200">
                      <p className="font-medium text-neutral-800">{companion.name}</p>
                      <p className="text-sm text-neutral-600 break-words">{companion.phoneNumber}</p>
                    </div>
                  ))}
                </div>
              </div>
